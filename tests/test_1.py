...
async def test_network_capture():
    ...
    try:
        ...
        3. 点击"获取用户数据"按钮，触发 API 调用

        4. 等待 API 响应（2秒）

        5. 使用 get_network_data 工具，传入参数 "user"，获取捕获的 API 数据

        6. 验证捕获的数据是否包含 username 和 points 字段
        ...
        # 验证捕获的数据
        print("\n🔍 验证捕获的数据：")
        captured = browser_manager.get_captured_data("user")
        if captured:
            print(f"✅ 成功捕获数据:")
            print(f"  URL: {captured['url']}")
            print(f"  数据: {captured['data']}")

            # 验证字段
            data = captured['data']
            if 'username' in data and 'points' in data:
                print(f"\n✅ 测试通过！捕获的数据包含所需字段")
            else:
                print(f"\n❌ 测试失败：数据缺少必需字段")
        else:
